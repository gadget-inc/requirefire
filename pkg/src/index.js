...
const createRequirefire = () => {
    ...
    function requireModule(request, parentModule) {
        ...
        let actualPath;
        try {
            actualPath = resolver.resolveSync({}, parentModule ? path_1.default.dirname(parentModule.filename) : ".", request);
        }
        catch (error) {
            const err = new Error(`Cannot find module '${request}'. Note: Using requirefire. Resolution error: ${error.message}`);
